...
class EvilConjurerSpec(_system: ActorSystem)
  ...
  "A listener" should {
    "receive a chained RideStart event" in {
      ...
      val fff = res.head._2.params.head._2
      val reflected = mirror.reflect(fff)
      val mathod = reflected.symbol.typeSignature.member(TermName("getClassName"))
      val className = reflected.reflectMethod(mathod.asMethod)().asInstanceOf[String]
      val clazz = Class.forName(className)
      //val res = reflected.reflectMethod(mathod.asMethodSymbol)(42)

      //Class.forName(res(0)._2.params.head._2.asInstanceOf[asm.Type].getClassName)
      val h = typeOf[EvilConjurer].typeSymbol.owner.info.decls
      val any = annotatedClasses
